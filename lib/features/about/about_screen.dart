            ...
            child: Column(
              ...
              children: [
                ...
                _Bullet('Овуляция — не фиксированные «−14», а лютеиновая '
                    'фаза по длине цикла (13–15 дней). Если вы ведёте '
                    'базальную температуру, Лея находит подтверждённый '
                    'подъём и вычисляет вашу личную длину этой фазы — '
                    'тогда прогноз опирается на неё, а не на среднее.', lea),
                _Bullet('Фертильное окно — 5 дней до овуляции и день овуляции '
                    ...
                    'фертильного периода. Если овуляция подтверждена '
                    'базальной температурой, этот день убирается — '
                    'поправка на погрешность больше не нужна.', lea),
                _Bullet('Если вы отмечаете тесты на овуляцию, положительный '
                    'результат становится главным: он означает пик гормона, '
                    'а овуляция наступает примерно через сутки. Такой '
                    'прогноз опирается на измерение, а не на расчёт, '
                    'и перекрывает календарную оценку.', lea),
